...
c_dat = {cBA ,cN1 ,cN1 ,cR1,cN2 ,cN1 ,cR2 ,cTT};
...
%% Average velocity in the second half of the cycle

fig6 = figure('color','w');

for d = 1:length(t_dat)
    vel_all = t_dat{d}.kph(~k_arr{d});
    vel_second_half = vel_all(floor(length(vel_all)/2):end);
    scatter(d, mean(vel_second_half), 80, c_dat{d}, 'filled')
    hold on
end
ax = gca;
% ax.Color = 'none';
...
box off
xlabel('Trial Number')
ylabel('Mean Velocity in the second half')
...
box off
xlabel('Trial Number')
ylabel('Mean Power in the second half')
...
box off
xlabel('Trial Number')
ylabel('Mean Cadence in the second half')
